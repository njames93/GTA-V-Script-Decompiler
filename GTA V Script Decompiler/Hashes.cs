...
namespace Decompiler
{
	public class Hashes
    {
        ...
        {			
            ...
            StreamReader reader;
            if (File.Exists(Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), "entities.dat")))
            {
                reader = new StreamReader(File.OpenRead(Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), "entities.dat")));
            }
            else
            {
        ...
        {
            Stream Decompressed = File.Create(Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), "entities_exp.dat"));
            Stream Compressed = new MemoryStream(Properties.Resources.Entities);
            DeflateStream deflate = new DeflateStream(Compressed, CompressionMode.Decompress);
            deflate.CopyTo(Decompressed);
            deflate.Dispose();
            Decompressed.Close();
            System.Diagnostics.Process.Start(Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location)));
